...
import models as _models, schemas as _schemas, config as _database
...
def get_user(db: _orm.Session, user_id: int):
    return db.query(_models.User).filter(_models.User.id == user_id)


def get_username(db: _orm.Session, username: str):
    return db.query(_models.User).filter(_models.User.id == username).first()
"""""
...
def create_user(db: _orm.Session, user: _schemas.UserCreate):
    ...
    db_user = _models.User(username=user.username, email=user.email, hashed_password=fake_hashed_password)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
